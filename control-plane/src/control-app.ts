...
export interface TaskStore {
  createTask(input: Pick<Task, "description" | "priority" | "requiredCapabilities"> & { id?: string }): Promise<Task>;
  getTask(id: string): Promise<Task | undefined>;
  transition(id: string, to: TaskStatus, reason: string, expectedVersion?: number): Promise<Task>;
  queue(id: string, executor: ExecutorKind, expectedVersion?: number): Promise<Task>;
  claim(agentId: string, capabilities: string[]): Promise<Task | undefined>;
...
export class InMemoryStore implements TaskStore {
  ...
  async transition(id: string, to: TaskStatus, reason: string, expectedVersion?: number): Promise<Task> {
    ...
    const now = new Date().toISOString();
    const changed = { ...current, status: to, version: current.version + 1, updatedAt: now };
  ...
  async queue(id: string, executor: ExecutorKind, expectedVersion?: number): Promise<Task> {
...
export function createApp(store: TaskStore, ownerToken?: string): Hono {
  ...
  app.post("/v1/hooks/codex", async (context) => {
    ...
    const taskId = `codex:${input.sessionId}`;
    ...
    if (input.eventName === "Stop" && task.status === TaskStatus.IN_PROGRESS) {
      return context.json(await store.transition(task.id, TaskStatus.VERIFYING, "codex_stop"));
    }
    if (
      input.eventName === "SessionStart" ||
